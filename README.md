...
**你不需要自己下指令**——首頁上方會出現一條橫幅，按「立即更新」就好：

- 下載工具**超過 90 天沒更新**時（yt-dlp 官方自己的發布週期就是 90 天）→ 黃色橫幅，**壞掉之前就先叫**
- 下載真的被擋時 → 紅色橫幅，不看天數
- 按下去程式只會更新**這個專案 venv 裡的 yt-dlp**，不動系統 Python、不動其他套件
- **有轉檔在跑時會拒絕更新**（換套件會影響正在跑的轉檔），等跑完再按
- 更新完如果需要重開，橫幅會直接講，並暫時停用「轉換」，免得你白跑一次

⚠️ **程式不會自動更新**，永遠是你按下去才更新——不在你不知情的時候改你的環境。

真的想手動下指令的話（**一定要用專案自己的 Python**，不然會裝到別的環境去）：
